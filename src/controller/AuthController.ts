...
export default () => {
  ...
  router.post("/authorize", async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const { user, token } = await getAccessTokenFromAuthCode({ userCode, email });

      const { refreshToken, userCode: code, confirmationCode, __v, ...rest } = user

      // set cookie
      const cookie = signCookie({ user, token }, token.expiresIn)

      res.cookie('token', cookie, { httpOnly: true, maxAge: token.expiresIn });
